...
def det(mod, fn, use_rpn_only=False):
    ...
    #im_tensor = image.transform(img, [103.53, 116.28, 123.675], 1.0)
    im_tensor = image.transform(img, [103.53, 116.28, 123.675], 0.017)

    im_info = np.array([[  IMG_H,   IMG_W,   1.0]])
...
if __name__ == '__main__':
    ...
    #testdir = '/home/kk/dev/dcn/data/plates/train/JPEGImages'
    #testdir = '/home/kk/dev/dcn/data/traffic_sign/train/JPEGImages'
    testdir = '/media/kk/WORK/proj/dev/veh/veh/data0'
    files = [i for i in os.listdir(testdir) if i.endswith('.jpg')]

    for i,fn in enumerate(files):
        #if i == 0:
        #    continue
        det(mod, testdir + '/' + fn, use_rpn_only=True)#'/'+ str(i) + '.jpg')